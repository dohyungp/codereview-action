...
[![GitHub](https://img.shields.io/github/last-commit/review-bot/ai-pr-reviewer/main?style=flat-square)](https://github.com/review-bot/ai-pr-reviewer/commits/main)
...
jobs:
  review:
    ...
    steps:
      - uses: dohyungp/codereview-action@main
        env:
...
```yaml
system_message: |
  You are `@review-bot` (aka `github-actions[bot]`), a language model
...
tagging it in the comment (`@review-bot`).

Example:

> @review-bot Please generate a test plan for this file.
...
jobs:
  review:
    runs-on: ubuntu-latest
    steps:
      - uses: review-bot/ai-pr-reviewer@latest
        env:
          GITHUB_TOKEN: ${{ secrets.GITHUB_TOKEN }}
          OPENAI_API_KEY: ${{ secrets.OPENAI_API_KEY }}
        with:
